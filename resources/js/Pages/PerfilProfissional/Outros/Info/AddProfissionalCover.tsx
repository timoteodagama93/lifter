...
function AddProfissionalCover({ artist: profissional }) {
  const page = useTypedPage();
  const route = useRoute();
  const { data, setData, progress, post, errors } = useForm({
    artist_id: profissional.id,
  ...
  function saveNewPost(e) {
    ...
    axios
      .post('/add-profissional-cover', formData)
      .then(response => {
        Swal.fire({
          title: 'Bem feito',
          text: 'A capa foi carregada.',
          icon: 'success',
        });
      })
      .catch(e => {
...
export default AddProfissionalCover;
