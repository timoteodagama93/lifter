...
function PlayingVideo({ playingVideo }) {
  function colecionar() {}
  ...
  useEffect(() => {
    ...
    data.append('song_id', playingVideo.id);
    axios
  ...
  return (
    <>
      <div className="w-full h-full flex flex-col overflow-hidden px-5 ">
        <div className="w-full h-[20%]  border-b px-1 md:px-5 flex flex-col md:flex-row justify-between items-center">
          <div className="w-full flex flex-row-reverse md:flex-row items-center gap-1">
            ...
            <span className="md:flex"> {playingVideo?.artist} </span>
          </div>
          <div className="w-full flex flex-row gap-1">
            <Interagir song={playingVideo} />{' '}
          </div>
        </div>
      ...
      >
        {valuateModal && (
          <Avaliar setOpenModal={setOpenModal} song={playingVideo} />
        )}
        {feedbackModal && (
          <Feedback setOpenModal={setOpenModal} song={playingVideo} />
        )}
        {commentsModal && (
          <Comentarios
        ...
        {shareModal && <Partilhar song={playingVideo} />}
      </Modal>
    </>
  );
}
